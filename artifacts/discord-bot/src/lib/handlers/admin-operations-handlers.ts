    ...
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription(
          ...
          "The bot will permanently delete the previous active gameday channel, create one new public league-wide gameday channel, and post reaction-based matchup panels with emoji controls.",
        ),
    ],
    components: [
    ...
    await reply({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `Created <#${result.channelId}> with reaction-based matchup panels and emoji controls.\n\n` +
            `H2H panels: **${result.h2hCount}**\n` +
            `Total games: **${result.totalGames}**` +
            (result.deletedPrevious ? "\nPrevious active gameday channel was replaced." : ""),
          )
          .addFields({ name: "Channel", value: result.channelUrl ? `[Open Channel](${result.channelUrl})` : `<#${result.channelId}>` })
          .setTimestamp(),
      ],
    });
  } catch (err) {
    console.error("[handlePostGameChannelsModal] reaction-based gameday creation failed:", err);
    await reply({ content: `❌ Weekly gameday channel creation failed: ${err}` });
  }
}
// ── Waitlist Hub ───────────────────────────────────────────────────────────────
