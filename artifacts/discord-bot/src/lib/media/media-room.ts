...
async function ensureMediaTables(): Promise<void> {
  ...
  await db.execute(sql`alter table media_goty_candidates add column if not exists rec_game_id bigint`);
  await db.execute(sql`alter table media_goty_candidates alter column away_discord_id drop not null`);
  await db.execute(sql`alter table media_goty_candidates alter column home_discord_id drop not null`);
  await db.execute(sql`alter table media_goty_candidates alter column away_score drop not null`);
  await db.execute(sql`alter table media_goty_candidates alter column home_score drop not null`);
  await db.execute(sql`alter table media_goty_candidates alter column winner_discord_id drop not null`);
  ...
  `);
  await db.execute(sql`
    create index if not exists media_goty_candidates_rec_game_idx
    on media_goty_candidates(rec_game_id)
  `);
...
async function showGotyWeekSelect(interaction: any): Promise<void> {
  ...
  const weekRows = await rowsOf<any>(sql`
    select distinct week_number
    from v_rec_goty_dropdown_source
    where guild_id = ${guildId}
      and week_number is not null
      and week_number between 1 and coalesce(${currentDisplayWeek ?? 18}, 18)
    order by week_number asc
  `).catch(() => []);

  const displayWeeks = weekRows
    .map((r) => Number(r.week_number))
    .filter((w) => Number.isInteger(w) && w >= 1 && w <= 22)
    .slice(0, 25);
  ...
  if (!displayWeeks.length) {
    await respondPanel(interaction, {
      ...
      embeds: [
        new EmbedBuilder()
          .setColor(Colors.Red)
          .setTitle("🎮 GOTY Nomination — No Imported H2H Weeks")
          .setDescription("No canonical H2H weeks are currently available for GOTY nomination. Run Data Diagnostics or a schedule import, then try again."),
      ],
      components: [mediaRoomBackRow()],
    });
    return;
  }
    ...
    .addOptions(
      displayWeeks.map((w) =>
        new StringSelectMenuOptionBuilder()
          ...
          .setDescription(w === currentDisplayWeek ? "Current active week" : "Previous active-season week")
          .setValue(String(w)),
      ),
    );
  ...
  await respondPanel(interaction, {
    ...
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription("Choose any available H2H week from the current active season. Current-week games may be nominated before final scores import."),
    ],
    components: [
...
async function showGotyMatchupSelect(interaction: any, displayWeek: number): Promise<void> {
  ...
  const games = await rowsOf<any>(sql`
    ...
    from v_rec_goty_dropdown_source
    where guild_id = ${guildId}
      and week_number = ${displayWeek}
      and away_discord_id is not null
      and home_discord_id is not null
    order by away_team_name asc, home_team_name asc, rec_game_id asc
    limit 25
  `).catch(() => []);
    ...
    await respondPanel(interaction, {
      ...
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(`No canonical H2H matchups were found for **${displayWeekLabel(displayWeek)}**.`),
      ],
      components: [
        new ActionRowBuilder<ButtonBuilder>().addComponents(
  ...
  await respondPanel(interaction, {
    ...
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription(`Showing canonical H2H matchups from **${displayWeekLabel(displayWeek)}**.`),
    ],
    components: [
      new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu),
      new ActionRowBuilder<ButtonBuilder>().addComponents(
...
async function showGotyWinnerSelect(interaction: any, scheduleId: number, displayWeek: number): Promise<void> {
  await showGotyNotesModal(interaction, displayWeek, scheduleId);
}

async function showGotyNotesModal(interaction: any, displayWeek: number, recGameId: number): Promise<void> {
  const modal = new ModalBuilder()
    .setCustomId(`mr_goty_notes:${displayWeek}:${recGameId}`)
  ...
  modal.addComponents(
    new ActionRowBuilder<TextInputBuilder>().addComponents(
      new TextInputBuilder()
        ...
        .setPlaceholder("Describe the comeback, rivalry stakes, big plays, drama, upset, OT finish, etc. Optional.")
...
async function handleGotyNotesModal(interaction: any, displayWeek: number, recGameId: number): Promise<void> {
  await ensureMediaTables();
  const guildId = interaction.guildId!;
  const season = await getOrCreateActiveSeason(guildId);
  ...
  const scoreLine = game.away_score != null && game.home_score != null
    ? `Score: ${game.away_team_name} ${game.away_score} — ${game.home_team_name} ${game.home_score}`
    : game.winner_discord_id
      ? "Score: not recovered from import yet"
      : "Score: awaiting import";

  const winnerLine = game.winner_discord_id
    ? `Winner: <@${game.winner_discord_id}>`
    : "Winner: awaiting import";
...
async function renderGotyNominees(interaction: any, voteMode = false, page = 0): Promise<void> {
  ...
  const rows = await rowsOf<any>(sql`
    select c.*,
           g.away_team_name as live_away_team_name,
           g.home_team_name as live_home_team_name,
           g.away_score as live_away_score,
           g.home_score as live_home_score,
           g.winner_discord_id as live_winner_discord_id,
           g.status as live_status
    ...
    order by c.week_index asc, c.created_at asc
    limit 1 offset ${safePage}
  `);
  ...
  const awayTeam = c.live_away_team_name ?? c.away_team_name ?? "Away";
  const homeTeam = c.live_home_team_name ?? c.home_team_name ?? "Home";
  const awayScore = c.live_away_score ?? c.away_score;
  const homeScore = c.live_home_score ?? c.home_score;
  const winner = c.live_winner_discord_id ?? c.winner_discord_id;
  const status = c.live_status ?? c.status;
  const scoreText = awayScore != null && homeScore != null
  ...
  const embed = new EmbedBuilder()
    .setColor(Colors.Gold)
    .setTitle(`🎮 GOTY Nominee ${safePage + 1}/${Math.max(total, 1)}`)
    .setDescription(
      [
        `**Week:** ${displayWeekLabel(Number(c.week_index ?? 0))}`,
        `**Game:** <@${c.away_discord_id}> (${awayTeam}) vs <@${c.home_discord_id}> (${homeTeam})`,
        `**Score:** ${scoreText}`,
        `**Winner:** ${winner ? `<@${winner}>` : "awaiting import"}`,
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(`mr_goty_page:${voteMode ? "vote" : "browse"}:${Math.max(0, safePage - 1)}`).setLabel("◀ Prev").setStyle(ButtonStyle.Secondary).setDisabled(safePage <= 0),
    ...
    new ButtonBuilder().setCustomId(`mr_goty_page:${voteMode ? "vote" : "browse"}:${safePage + 1}`).setLabel("Next ▶").setStyle(ButtonStyle.Secondary).setDisabled(safePage >= total - 1),
    new ButtonBuilder().setCustomId("mr_goty_home").setLabel("GOTY Home").setStyle(ButtonStyle.Secondary),
  );

  await respondPanel(interaction, { ephemeral: true, embeds: [embed], components: [row, mediaRoomBackRow()] });
}
...
export async function handleMediaRoomInteraction(interaction: any): Promise<boolean> {
  ...
  if (id.startsWith("mr_goty_matchup:")) { await showGotyNotesModal(interaction, Number(id.split(":")[1]), Number(interaction.values[0])); return true; }
  if (id.startsWith("mr_goty_winner:")) {
    const [, , displayWeekRaw, scheduleIdRaw] = id.split(":");
    await showGotyNotesModal(interaction, Number(displayWeekRaw), Number(scheduleIdRaw));
    return true;
  }
  if (id.startsWith("mr_goty_notes:")) {
    const [, , displayWeekRaw, recGameIdRaw] = id.split(":");
    await handleGotyNotesModal(interaction, Number(displayWeekRaw), Number(recGameIdRaw));
    return true;
  }
