...
export const purchasesTable = pgTable("purchases", {
  ...
  purchaseGuildSeasonStatusIdx: index("purchases_guild_season_status_idx").on(t.guildId, t.seasonId, t.status),
  purchaseGuildUserSeasonIdx: index("purchases_guild_user_season_idx").on(t.guildId, t.discordId, t.seasonId),
  purchaseGuildTypeStatusIdx: index("purchases_guild_type_status_idx").on(t.guildId, t.purchaseType, t.status),
}));
...
export const inventoryTable = pgTable("inventory", {
  ...
  inventoryGuildUserSeasonIdx: index("inventory_guild_user_season_idx").on(t.guildId, t.discordId, t.seasonId),
  inventoryGuildTeamSeasonIdx: index("inventory_guild_team_season_idx").on(t.guildId, t.team, t.seasonId),
  inventoryGuildPurchaseIdx: index("inventory_guild_purchase_idx").on(t.guildId, t.purchaseId),
}));
...
export const seasonStatsTable = pgTable("season_stats", {
  ...
  seasonStatsGuildUserSeasonIdx: uniqueIndex("season_stats_guild_user_season_idx").on(t.guildId, t.discordId, t.seasonId),
}));
...
export const customPlayersTable = pgTable("custom_players", {
  ...
  customPlayersGuildSeasonUserIdx: index("custom_players_guild_season_user_idx").on(t.guildId, t.seasonId, t.discordId),
  customPlayersGuildStatusIdx: index("custom_players_guild_status_idx").on(t.guildId, t.status),
}));


// ── Custom Player Wizard Sessions (durable routing/state checkpoint) ──────────
export const customPlayerWizardSessionsTable = pgTable("custom_player_wizard_sessions", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  guildId: text("guild_id").notNull(),
  ...
  selectedArchetypeId: bigint("selected_archetype_id", { mode: "number" }),
  selectedArchetypeName: text("selected_archetype_name"),
  packageTier: text("package_tier"),
  devTrait: text("dev_trait"),
  state: json("state").notNull().default({}).$type<Record<string, unknown>>(),
  expiresAt: timestamp("expires_at", { withTimezone: true }).notNull().defaultNow(),
  submittedCustomPlayerId: bigint("submitted_custom_player_id", { mode: "number" }),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => ({
  customPlayerWizardSessionIdx: uniqueIndex("custom_player_wizard_sessions_session_idx").on(t.guildId, t.sessionId),
  customPlayerWizardUserActiveIdx: index("custom_player_wizard_sessions_user_active_idx").on(t.guildId, t.discordId, t.status, t.expiresAt),
  customPlayerWizardCleanupIdx: index("custom_player_wizard_sessions_cleanup_idx").on(t.status, t.expiresAt),
}));
...
// ── Consolidated Discord identity / guild-franchise ownership layer ─────────
// Added by consolidated rebuild phase 4. These tables do not replace legacy
// economy/team tables immediately; they normalize identity so future features
// resolve users and guild membership from one source of truth.
...
export const guildMembershipsTable = pgTable("guild_memberships", {
  id: serial("id").primaryKey(),
  guildId: text("guild_id").notNull(),
  discordId: text("discord_id").notNull(),
  displayName: text("display_name"),
  serverNickname: text("server_nickname"),
  isAdmin: boolean("is_admin").notNull().default(false),
  ...
  uniqGuildMember: uniqueIndex("guild_memberships_guild_discord_idx").on(t.guildId, t.discordId),
}));

export const guildFranchisesTable = pgTable("guild_franchises", {
  id: serial("id").primaryKey(),
  guildId: text("guild_id").notNull(),
  seasonId: integer("season_id").notNull(),
  eaLeagueId: integer("ea_league_id"),
  ...
  uniqGuildSeason: uniqueIndex("guild_franchises_guild_season_idx").on(t.guildId, t.seasonId),
}));

export const guildFranchiseUserTeamsTable = pgTable("guild_franchise_user_teams", {
  id: serial("id").primaryKey(),
  guildId: text("guild_id").notNull(),
  seasonId: integer("season_id").notNull(),
  ...
  uniqGuildSeasonTeam: uniqueIndex("guild_franchise_user_teams_team_idx").on(t.guildId, t.seasonId, t.teamName),
  uniqGuildSeasonDiscord: uniqueIndex("guild_franchise_user_teams_discord_idx").on(t.guildId, t.seasonId, t.discordId),
}));

export type DiscordUser = typeof discordUsersTable.$inferSelect;
export type GuildMembership = typeof guildMembershipsTable.$inferSelect;
export type GuildFranchise = typeof guildFranchisesTable.$inferSelect;
export type GuildFranchiseUserTeam = typeof guildFranchiseUserTeamsTable.$inferSelect;

// ── Phase 5 guild routing / feature config / operational audit layer ─────────
// Non-destructive consolidation tables. These provide a central routing/config
// surface while existing server_settings and handler-specific tables continue to
// work during feature-by-feature extraction.
export const guildDiscordRoutesTable = pgTable("guild_discord_routes", {
  id: serial("id").primaryKey(),
  guildId: text("guild_id").notNull(),
  ...
  uniqGuildRoute: uniqueIndex("guild_discord_routes_unique_idx").on(t.guildId, t.routeKey),
}));

export const guildFeatureConfigTable = pgTable("guild_feature_config", {
  id: serial("id").primaryKey(),
  guildId: text("guild_id").notNull(),
  featureKey: text("feature_key").notNull(),
  enabled: boolean("enabled").notNull().default(true),
  config: json("config").$type<Record<string, unknown>>().notNull().default({}),
  source: text("source").notNull().default("consolidated-rebuild"),
  updatedBy: text("updated_by"),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => ({
  uniqGuildFeature: uniqueIndex("guild_feature_config_unique_idx").on(t.guildId, t.featureKey),
}));

export const guildFranchiseGameLinksTable = pgTable("guild_franchise_game_links", {
  id: serial("id").primaryKey(),
  guildId: text("guild_id").notNull(),
  seasonId: integer("season_id").notNull(),
  weekIndex: integer("week_index").notNull(),
  matchupKey: text("matchup_key").notNull(),
  franchiseScheduleId: bigint("franchise_schedule_id", { mode: "number" }),
  ...
  uniqGuildMatchupLink: uniqueIndex("guild_franchise_game_links_unique_idx").on(t.guildId, t.seasonId, t.weekIndex, t.matchupKey),
}));

export const guildOperationalEventsTable = pgTable("guild_operational_events", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  guildId: text("guild_id").notNull(),
...
export type GuildDiscordRoute = typeof guildDiscordRoutesTable.$inferSelect;
export type GuildFeatureConfig = typeof guildFeatureConfigTable.$inferSelect;
export type GuildFranchiseGameLink = typeof guildFranchiseGameLinksTable.$inferSelect;
export type GuildOperationalEvent = typeof guildOperationalEventsTable.$inferSelect;
